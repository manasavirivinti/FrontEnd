...
const REDIRECT_URI = "http://localhost:443/linkedin/callback";
...
const Login=({setAuth })=>{
  ...
  function handleCallBackResponse(response){
    ...
    // fetch("http://localhost:443/auth/login", {
    //     method: "POST",
    //     headers: { "Content-Type": "application/json" },
    //     body: JSON.stringify(body)
        
    // })
    fetch("https://deployment-8nty.onrender.com/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body)
...
//    };
   ...
   const handleLinkedInLogin = () => {
    window.location.href = `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=86zw1ufreib25c&scope=profile&state=someRandomString&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Flogin`;
};
...
//                     code: code,
//                     redirect_uri: "http://localhost:6000/linkedin/callback",
//                     client_id: "86zw1ufreib25c",
//                     client_secret: "WuSbXDlt00DGRRyw",
//                 };
...
//                 const data = await response.json();
//                 console.log(data);
//                 const { authenticated, role } = await authenticateWithBackend(data.access_token);
//                 console.log(role);
...
useEffect(() => {
    // Check if the URL contains the authorization code from LinkedIn
    const handleLinkedInCallback = async () => {
        const code = new URLSearchParams(window.location.search).get("code");
        if (code) {
            try {
                // Make a request to your backend to exchange the authorization code for an access token
                const response = await fetch(`http://localhost:443/linkedin/callback?code=${code}`);
                const data = await response.json();
                // Check if authentication was successful
                console.log(data);
                // if (data.authenticated) {
                //     // Set authentication status and redirect user based on their role
                //     setAuth(true);
                //     if (data.role === 'ADMIN') {
                //         window.location.href = "/admin";
                //     } else {
...
async function authenticateWithBackend(accessToken) {
    try {
        const response = await fetch("http://localhost:443/auth/linkedin/callback", {
            method: "POST",
            headers: {
